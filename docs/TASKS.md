...
- Tests: new `test/prtfConditionedKeywords.test.ts`, 11 tests — parser
  ...
  **Merged with Batch AA's concurrently-landed column-6/`formType` work
  (this batch's `emitEntryWithConditionedKeywords` and Batch AA's
  `formType` param both needed to thread through the same call sites) —
  `formType` is now also passed down onto each attached conditioned-
