...
| `CPI` | **10 or 15 only** | Not a free-form number as the current requirements doc implied — confirmed fixed choice list on the RLU screen. Record-level-or-field-level only (no file-level DDS keyword form — see `LPI`'s note below, same fix). Batch AAA (`docs/TASKS.md`): a field's own `CPI`, when present, is now exposed on its cell (`cell.cpi`) and, when it genuinely differs from the record's resolved `CPI`, surfaces a `fieldWarnings` note — the rendered preview still lays every entry out on the record's one shared character grid (real per-field density differences aren't reflected visually), so the note explains the approximation rather than silently ignoring the divergence.
...
| `LPI` | **4, 6, 8, 9, or 12 only** | Fixed choice list, not free-form. Record-level only (no file-level DDS keyword form — Batch AAA, `docs/TASKS.md`: `resolveCpiLpi` used to also fall back to a file-level keyword line for both `CPI` and `LPI`, which could never legitimately exist in valid DDS; both now fall straight through to their hardcoded default — 10/6 — instead, same fix shape as `PAGSIZE`/`DEVTYPE` before Batch SS). |
