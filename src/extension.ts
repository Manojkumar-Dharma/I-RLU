...
// eslint-disable-next-line @typescript-eslint/no-var-requires
const AfpResourceDecoder = require("./afpResourceDecoder.js");
  ...
  public async resolveCustomTextEditor(
    ...
    webviewPanel.webview.onDidReceiveMessage(async (msg: WebviewMessage) => {
      ...
      if (msg.type === "addFieldsFromDatabase") {
        await handleAddFieldsFromDatabase(document, currentModel, msg, { enabled: modTrackingEnabled, tag: modTrackingTag });
        currentModel = parseSource(document.getText());
        postModel();
        await sendCodeForIStatus();
      }
      if (msg.type === "previewAfpResource") {
        await handlePreviewAfpResource(webviewPanel, msg);
      }
    });
  }
 ...
 * once at the end (via applyTrackedDocumentEdit, Batch X) — unlike
 * I-SDA's own `handleAddFieldsFromDatabase` (which re-parses the document
 * TEXT after each field, since its DspfWriter.insertField works at the
 * line-text level), I-RLU's `applyEditToModel` mutates model objects
 * directly (record.fields/model.sequence), so each successive `addField`
 * call already sees the previous one's result with no text round-trip
 * needed in between — one `regenerateSource` + one WorkspaceEdit for the
 * whole batch, so VS Code's undo stack sees it as a single action.
 */
...
/**
 * Handles a 'previewAfpResource' message (Batch O, docs/TASKS.md): the
 * OVERLAY/PAGSEG/AFPRSC keyword's own "Preview resource image…" button.
 * Real page-segment/overlay resource FILES live on the IBM i's IFS/host,
 * not in DDS source text (docs/REQUIREMENTS.md §8), and this project has
 * no Code-for-i-based IFS file browser yet (unlike DSPFFD-based database
 * queries — see fetchDatabaseFileFields — there's no established pattern
 * for browsing arbitrary IFS objects here) — so this shows a native local
 * file picker instead, for a resource file the person has a local copy
 * of. Reads and decodes it via afpResourceDecoder.js and posts the result
 * straight back to the SAME webview panel (not through a document edit —
 * previewing a resource doesn't change the DDS source at all) as an
 * "afpResourcePreview" message.
 *
 * Never throws back to its caller — decode failures (an unsupported IOCA
 * function set, a text/graphics-only overlay with no image content at
 * all) are real, expected outcomes given this module's own stated scope
 * (see afpResourceDecoder.js's own header comment), not something the
 * person picking a file did wrong; they're shown as the SAME kind of
 * inline message a decode success would be, not an error dialog.
 */
async function handlePreviewAfpResource(webviewPanel: vscode.WebviewPanel, msg: { recordName: string; keyword: "OVERLAY" | "PAGSEG" | "AFPRSC" }): Promise<void> {
  const picked = await vscode.window.showOpenDialog({
    canSelectMany: false,
    openLabel: "Preview as " + msg.keyword,
    title: "Select a local AFP page-segment/overlay resource file",
    filters: { "AFP resource": ["afp", "301", "ovl", "pseg"], "All files": ["*"] },
  });
  if (!picked || picked.length === 0) return;

  let result: { name?: string; width?: number; height?: number; dataUri?: string; error?: string };
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const fs = require("fs");
    const buffer = fs.readFileSync(picked[0].fsPath);
    result = AfpResourceDecoder.decodeAfpResourceToPngDataUri(buffer);
  } catch (e) {
    result = { error: e instanceof Error ? e.message : String(e) };
  }
  webviewPanel.webview.postMessage({
    type: "afpResourcePreview",
    recordName: msg.recordName,
    keyword: msg.keyword,
    fileName: picked[0].fsPath.split(/[\\/]/).pop(),
    result,
  });
}
